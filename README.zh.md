...
<p align="center">
  面向同时使用多个 AI 编程代理团队的可移植 agent packs、runtime adapters
  和 orchestration contracts。
</p>
...
> This Chinese README is a localized quick guide. The English
> [README.md](README.md) remains the canonical reference for the full project
> documentation.

## 什么是 Craft Harness

Craft Harness 不是另一个单独的 coding agent。它是围绕 Claude、Codex、
Gemini、OpenCode 和 OpenHands-style workflows 的 harness layer。

- 可复用的 `agents/`、`skills/`、`commands/`
- 面向不同 runtime 的 guidance adapters
- 基于 git worktree 和 tmux 的 worker DAG
- success criteria、eval type、handoff artifacts
- 面向公开仓库的 validation 和 secret scan

## 适合谁使用

- 在同一个代码库中并行使用多个 AI 编程工具的团队
- 希望把 prompt、role、skill 变成可复用 pack 的开发者
- 希望把 QA、review、acceptance criteria 纳入 agent workflow 的团队
- 使用 git worktree 管理并行 agent 工作的用户
...
Install commands 默认不会覆盖已有文件。使用 `--dry-run` 预览变更；只有在明确想替换
adapter 或 skill directory 时才使用 `--force`。
...
完整 plan contract 请参考 [Spec and Contract Notes](docs/spec-contract.md)。
